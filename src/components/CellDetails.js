...
class CellDetails extends React.Component {
  ...
  computeSuggestions() {
    ...
    this.columnSuggestions = getKDigitCombinations(columnDigits.length).filter(
      (c) => digitSum(c) === columnSum
    );

    const rowSum = (rowData && rowData.sum) || 0;
    this.rowSuggestions = getKDigitCombinations(rowDigits.length).filter(
      (c) => digitSum(c) === rowSum
    );
  }
  ...
  render() {
    ...
    const scores = pool.map((digit) =>
      suggestions.reduce((ps, comb) => ps + comb.includes(digit), 0)
    );
    ...
    return (
      <div className="cellDetails">
        ...
        <div className="attributes">
          <span className="xVal"> X = {x}</span>
          <span className="yVal"> Y = {y}</span>
          <span className="valueVal"> VALUE = {value}</span>
        </div>
        <div className="options">
          {pool.map((digit, idx) => {
            ...
            return (
              <div
                className={
                  !this.options.includes(digit) ? "option disabled" : "option"
                }
